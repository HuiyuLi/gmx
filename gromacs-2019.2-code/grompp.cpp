...
/* added by Huiyu Li, 2023-01-23, perturb initial velocity */
void hl_getmass(gmx_mtop_t *sys, real *mass)
{
  int i;
  gmx_mtop_atomloop_all_t aloop;
  const t_atom           *atom;
...
real hl_calc_temp(rvec v[], real *mass, int natoms, real* kin)
{
  real boltz, temp;
  int i, m, nrdf;
  ...
  for (i = 0; i < natoms; i++){
    for (m = 0; m < DIM; m++){
      kin[0] += 0.5 * mass[i] * v[i][m] * v[i][m];
    } 
  }

  temp = (2.0 * kin[0]) / (nrdf * BOLTZ);
  fprintf(stderr, "HL hl_calc_temp atom %d %d with kin %lf temp %lf BOLTZ %lf\n", natoms, nrdf, kin[0], temp, BOLTZ);
  return temp;
}

/* scale velocity to target temperature */
void hl_scale_velocity(rvec v[], real *mass, int natoms, real tempi)
{
  int i, m;
  real tempnew, scale, kin;

  tempnew = hl_calc_temp(v, mass, natoms, &kin);
  scale = sqrt(tempi / tempnew);
  for (i = 0; i < natoms; i++){
    for (m = 0; m < DIM; m++){
      v[i][m] *= scale;
    }
  } 
  fprintf(stderr, "HL scale velocity %lf to %lf by scale %lf\n", tempnew, tempi, scale);
  tempnew = hl_calc_temp(v, mass, natoms, &kin);
}

/* perturb velocity by adding random vector */
void hl_perturb_addRand(rvec v[], real *mass, int natoms, gmx_mtop_t *sys,
                        real perturbsize, int perturbseed, rvec x[])
{
  real tempi, tempp, kini;
  rvec *newvel;
  int i, m;

  fprintf(stderr, "\nHL perturb velocity by add random vector\n");
  tempi = hl_calc_temp(v, mass, natoms, &kini);
  tempp = tempi * perturbsize;
  snew(newvel, natoms);
  maxwell_speed(tempp, perturbseed, sys, newvel);
  for (i = 0; i < natoms; i++){
    for (m = 0; m < DIM; m++){
      v[i][m] += newvel[i][m];
    }
  ...
  sfree(newvel);
  fprintf(stderr, "HL before stop_cm %lf\n", v[0][0]);
  stop_cm(stdout, natoms, mass, x, v);
  fprintf(stderr, "HL after stop_cm %lf\n", v[0][0]);
  hl_scale_velocity(v, mass, natoms, tempi);
}

/* random number within [low, up] with uniform distribution */
real hl_uniformDist(real low, real up)
{
  real num;
  real scale, shift;
  ...
  scale = up - low;
  shift = low;
  num = scale * ((real) rand() / (real) RAND_MAX) + shift ;

  if ((num < low) || (num > up)){
    fprintf(stderr, "\nHL Error: num %lf out of range [%lf %lf]\n\n", num, low, up);
    exit(1);
  } 

  /*fprintf(stderr, "HL random %lf between [%lf %lf]\n", num, low, up);*/
  return num;
}

/* perturb velocity by randomly scale each element of velocity vector */
void hl_perturb_scale(rvec v[], real *mass, int natoms, gmx_mtop_t *sys,
                      real perturbsize, int perturbseed, rvec x[])
{
  real scale, tempi, kin, low, up;
  int i, m;

  fprintf(stderr, "\nHL perturb velocity by scaling velocity with random number\n");
  tempi = hl_calc_temp(v, mass, natoms, &kin);
  low = 1.0 - perturbsize;
  up  = 1.0 + perturbsize;
  srand(perturbseed);
  for (i = 0; i < natoms; i++){
    for (m = 0; m < DIM; m++){
      scale = hl_uniformDist(low, up);
      v[i][m] *= scale;
    }
  }
  fprintf(stderr, "HL before stop_cm %lf\n", v[0][0]);
...
/* perturb velocity by add random number from Gaussian with 0 mean and perturbsize STD */
/* then scale this value by the value of initial velocity, then add it to initial velocity */
void hl_perturb_Gaussian(rvec v[], real *mass, int natoms, gmx_mtop_t *sys,
                         real perturbsize, int perturbseed, rvec x[])
{
  real tempi, kini;
  rvec *newvel;
  ...
  fprintf(stderr, "\nHL perturb velocity by add Gaussian random number for each U\n");

  fpvel = fopen("newvel.dat", "w");
  tempi = hl_calc_temp(v, mass, natoms, &kini);
  snew(newvel, natoms);
  hl_gaussian(newvel, natoms, perturbsize, perturbseed);
  for (i = 0; i < natoms; i++){
    for (m = 0; m < DIM; m++){
      v[i][m] += v[i][m] * newvel[i][m];
      fprintf(fpvel, "%d %lf\n", i*3+m, newvel[i][m]);
    }
  }
  sfree(newvel);
...
#define NFILE asize(fnm)
    ...
    /* added by Huiyu Li before tpr file output, 2023-01-23 */
    real *mass;
    int natoms, perturbatom, perturbmethod;
    natoms = sys.natoms;
    fprintf(stderr, "\nHL perturbmethod %d perturbsize %lf for %d atoms\n", 
                    ir->bias.perturbmethod, ir->bias.perturbsize, natoms);
    if (ir->bias.perturbmethod == 0){
      /* no perturbation */
      fprintf(stderr, "HL perturbmethod %d, no perturbation of initial velocity\n\n",
                      ir->bias.perturbmethod);
    } else{
      snew(mass, natoms);
      hl_getmass(&sys, mass);

      if (ir->bias.perturbatom == -1){
        perturbatom = natoms;
      } else{
        perturbatom = ir->bias.perturbatom;
      }
      fprintf(stderr, "Actually we perturb %d atoms\n", perturbatom);

      if (ir->bias.perturbmethod == 1){
        /* perturb velocity by adding a random vector with small magnitude */
        hl_perturb_addRand(state.v.rvec_array(), mass, perturbatom, &sys,
                           ir->bias.perturbsize, ir->bias.perturbseed, 
                           state.x.rvec_array());
      } else if (ir->bias.perturbmethod == 2){
        hl_perturb_scale(state.v.rvec_array(), mass, perturbatom, &sys,
                         ir->bias.perturbsize, ir->bias.perturbseed,
                         state.x.rvec_array());
      } else if (ir->bias.perturbmethod == 3){
        hl_perturb_Gaussian(state.v.rvec_array(), mass, perturbatom, &sys,
                            ir->bias.perturbsize, ir->bias.perturbseed,
                            state.x.rvec_array()); 
      } else{
        fprintf(stderr, "\nHL Error: no implementation for perturbmethod %d\n\n", 
			ir->bias.perturbmethod);
        exit(1)	;
      }
      sfree(mass);
    }
    fprintf(stderr, "HL finish perturbation\n\n");
    /* End of adding by Huiyu Li */
