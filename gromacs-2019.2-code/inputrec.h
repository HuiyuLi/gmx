...
/* added by Huiyu Li, add bias force on dihedral */
/* support at most 2000 dihedrals, each dihedral has 4 atoms */
/* for each dihedral, at most 200 bias bins */
#define HLATOM			4
#define HLLENNM			2000

struct BIC 
{
  char typeic[10];      /* d for dihedral */
  int atom[HLATOM];     /* atoms involved in the IC */
  double strength;      /* strength of the harmonic bias potential */
  int direction;        /* 1 for positive direction bias, -1 for negative */
  
  double pot;           /* potential energy of bias potential on this IC */
  double val;           /* coordinate of the IC */
  double eq;            /* equilibrium location of bias force */

  /* each IC has individual harmonic bias potential */
  int numbin;           /* number of bias bin */
  double binsize;       /* width of bias bin */
  double *bd;           /* bias bin boundaries */
  double low;
  double up;            /* range of bias bins are [low, up] */

  int currbin;          /* index of the bias bin, in which IC locates */
  int fastbin;          /* the index of the furthest" bin, where system ever reaches */

  /* we can add bias force on cosine of dihedral */
  /* this is to deal with the -Pi and Pi boundary crossing of dihedral */
  double cosval;        /* cos(dih) */
  double pineg;         /* no bias force when dihedral is between [-180, pineg] */
  double pipos;         /* no bias force when dihedral is between [pipos, 180] */

  /* cb is for conditional bias, e.g., alanine dipeptide, this is the BIC of theta_1 */
  /* add bias on theta_1, only when phi reaches certain range */
  /* then cbic in BIC of theta_1 is the index of phi (this index is the index of phi in mdp file) */
  /* If mdp file involves 3 dihedral, phi, theta_1, psi */
  /* then index of phi in mdp file is 0 */
  int cbic;             /* the index of phi in example */
  double valcbic;       /* the value of phi */
  double lowcbic;
  double upcbic;        /* [lowcbic, upcbic] is a range for phi, only when phi reaches this range, bias force will be added on theta1 */

  /* collective variable (CV) */
  /* gbf correspond to different expression of bias force and CV */
  /* For example, gbf == 1, CV = sum_i [coeff * (cvshift + cos(val - cvcenter))] */
  double cvcenter;      /* target value of dihedral */ 
  double cvcoeff;       /* coefficient of dihedral */ 
  double cvshift;       /* shift value for dihedral */
 
  /* whether the dihedral is order paramters or not */
  /* if yes, we would output its value to file for each time step */
  /* and it would be used to check whether system reaches target basin or not */
  int od;      
  double odup;
  double odlow;         /* [odlow, odup] is the range of dihdral in target basin */ 

  double neggenef;      /* negative of value of generalized bias force on this dihedral  */
};                   /* one IC, one BIC */

struct BIAS 
{
  /* added by Huiyu Li, adding bias force on dihedral, Jun 23 2022 */
  /* support explicit solvent and MPI (multiple cores within one nodes) */
  int numic;         /* number of internal coordinates to add bias force */
  BIC *bic;
  rvec *biasf;       /* bias force on Cartesian coordinates */
  int whecv;         /* whether we use collective varible, 0 for no, 1 for yes */
                     /* whe whecv == -1, we do cosine for each dihedral, not CV */
  double cv;         /* the value of collective varible */
  int cvnumbin;      /* number of bins for collective variables */
  int cvstep;        /**/
  int cvdirection;   /* during running, based on initial condition and */
                     /* cvtarget, to determine direction of bias force on CV */
  double cvup;
  double cvlow;      /* range of CV */
  double cvstrength; /* strength of bias force on CV */
  double cvtarget;   /* when cv reaches this value, top the simulation */
  double *cvbd;      /* bin boundaries and bin size for CV*/
  double cvbinsize;
  double cveq;
  int cvcurrbin;    /* the bin in which CV locates */
  int cvfastbin;    /* the furtherest bin that CV ever reaches */

  int whebiasstop;  /* whether store the simulation when order parameters reaches the other basin */
  int whereach;     /* whether the system reaches the other basin or not, use order parameters */
  int gbf;       /* generalized bias force analytical expression, different integer correspond to different bias force analytial expression */

  int numproteinatom;  /* we would have trr file with protein atoms only */
  int outfreq;         /* output frequency for default entire system trr file */
  FILE *fpbasin;       /* output basin information, value of order parameters */
  FILE *fpbf;          /* output bias force */
  FILE *fpbias;        /* output bias information */

  int everbasin;       /* whether system reaches basin before */
  gmx_bool bLastStep;  /* to stop simulation if needed */

  int natom;           /* number of atoms of system */

  /* output protein atoms into a new trr file */  
  int selftrr;         /* 1 for output a new trr file, 0 for no */

  int selfnatom;       /* number of atoms we want to output into the new trr */
                       /* e.g. HIV with ligand, protein+ligand has 3280 atoms */
                       /* new trr file involves atom 0~3279 */

  int deffreq;         /* The nstxout, nstvout, nstfout in mdp file */
                       /* will be used to control output frequency of our new trr */
                       /* The Gromacs trr will be controlled by deffreq */ 

  /* added by Jul 20 2022, for CV, Left/Right bias boundary should be used, -1 for left, 1 for right */
  /* 0 for "auto", -100 means not used */
  int lrbb;            /* left or right bin boundary */
  int signU;            /* sign of U, 1 for along U vector direction, -1 for opposite direction */

  /* whether calculate value of BBdih involed in U coordinates */
  int wheU;             /* 0 for not, otherwise it is number of BBdih */
  double *ubbdih;      /* value of BBdih at one time step */
  char nmu[HLLENNM];    /* name of file involving atom list of BBdih */
  int **uatom;          /* list of atoms involved, size is (wheU * 4), each BBdih has 4 atoms */
  FILE *fpdih;          /* file pointer for output these BBdih */

  /* box size */
  rvec boxsize;

  /* Perturb initial velocity */
  int perturbmethod;   /* different methods for purturbation */
                       /* 0: do not perturb */
                       /* 1: generate a new vector with smaller magnitude, add new vector to */
                       /*    the original velocity vector, */
                       /*    then scale velocities according to original temperature  */
                       /* 2: For each dimension of original velocity, multiply it by a random number */
                       /*    For 20% perturbation, the random number should be in range (0.8, 1.2) */
                       /*    then scale velocities according to original temperature */
  real perturbsize;    /*between 0 and 1, like 20% */
  int perturbseed;     /* random seed for random perturbation */
  int perturbatom;     /* we may perturb protein atoms only, -1 means entire system */
};
