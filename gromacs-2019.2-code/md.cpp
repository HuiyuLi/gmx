...
void gmx::Integrator::do_md()
{
    ...
    fprintf(stderr, "\nHL numic %5d natom %d gbf %d\n", ir->bias.numic, state->natoms, ir->bias.gbf);
    int hli, hlj;
    FILE *fptest;
    int hlstep;
    char *hldir;
    ...
    rvec *hlforce, *hlx;
    BIAS *bias;
    gmx::ArrayRefWithPadding<gmx::RVec> hlpdf, hlpdx;
    if (ir->bias.numic > 0){
      bias = &(ir->bias);
      hlstep = 0;
      hldir = hl_parse_currentdir(&outf);
      fprintf(stderr, "\nHL hldir %s\n\n", hldir);
      snprintf(hlfnm, 800, "%s/xvf.dat", hldir);
      fptest = fopen(hlfnm, "w");
      initializeBIAS(ir, state->natoms, hldir);

      for (hli = 0; hli < 3; hli++){
        bias->boxsize[hli] = state->box[hli][hli];
        fprintf(stderr, "HL boxsize %5d %lf\n", hli, bias->boxsize[hli]);
      }
    } 
    /* initialize self trr */
    init_self_mdoutf(&outf, ir, &hlstep, hldir);
    ...
    while (!bLastStep)
    {
        ...
        else
        {
            ...
            /* added by Huiyu Li for testing */
            if (ir->bias.numic > 0){
              if ((hlstep % ir->bias.deffreq) == 0){
                fprintf(stderr, "\nHL do_force t %lf\n", t); /* this shows up only once at each time point, when we use 16 CPUs */
              }  

              hlpdf = f.arrayRefWithPadding();
              hlforce = as_rvec_array(hlpdf.unpaddedArrayRef().data());
              hlpdx = state->x.arrayRefWithPadding();
              hlx = as_rvec_array(hlpdx.unpaddedArrayRef().data());
  
              /* add bias force, implementation in mdout.cpp */
              add_bias_force(ir, hlforce, hlx, t, hlstep);
              if ((!bLastStep) && (ir->bias.whebiasstop == 1) && (ir->bias.bLastStep)){
                fprintf(stderr, "\nHL bias reaches target basin, stop simulation at t = %lf ps\n", t);
                bLastStep = TRUE;
              }        

              /* output xvf used for bias calculation */
              /* this will be compared with trr file to see whether we use correct x */
              /*
	      if ((hlstep % ir->bias.deffreq) == 0){
                fwrite(&t, sizeof(double), 1, fptest);
                for (hli = 0; hli < state->natoms; hli++){
                  for (hlj = 0; hlj < 3; hlj++){
                    fwrite(&hlx[hli][hlj], sizeof(double), 1, fptest);
                    fwrite(&hlforce[hli][hlj], sizeof(double), 1, fptest);
                  }
                }
              }
	      */
