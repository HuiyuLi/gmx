...
/* added by Huiyu Li, add bias force */
static void do_bias_ir(t_fileio *fio, BIAS *bias, gmx_bool bRead, int file_version)
{
  int i, j;
  BIC *currbic;
  fprintf(stderr, "\nHL tpxio.cpp file_version %d\n\n", file_version);
 
  if (bRead){
    fprintf(stderr, "  HL read tpr file\n\n");
  } else{
    fprintf(stderr, "  HL write tpr file\n\n");
  }
 
  gmx_fio_do_int(fio, bias->numic);
  ...
  for (i = 0; i < bias->numic; i++){
    currbic = &(bias->bic[i]);
    gmx_fio_do_string(fio, bias->bic[i].typeic);
    for (j = 0; j < HLATOM; j++){
      gmx_fio_do_int(fio, bias->bic[i].atom[j]);
    }
    gmx_fio_do_int(fio,bias->bic[i].numbin);

    if (bRead){
      currbic->bd = (double *) malloc((bias->bic[i].numbin+1) * sizeof(double)); 
    }

    gmx_fio_do_real(fio,bias->bic[i].low);
    gmx_fio_do_real(fio,bias->bic[i].up);
    gmx_fio_do_real(fio,bias->bic[i].strength);
    gmx_fio_do_int(fio,bias->bic[i].direction);
    gmx_fio_do_int(fio,bias->bic[i].cbic);
    gmx_fio_do_real(fio,bias->bic[i].lowcbic);
    gmx_fio_do_real(fio,bias->bic[i].upcbic);

    gmx_fio_do_int(fio, bias->bic[i].od);
    gmx_fio_do_real(fio, bias->bic[i].odlow);
    gmx_fio_do_real(fio, bias->bic[i].odup);

    gmx_fio_do_real(fio, bias->bic[i].pineg);
    gmx_fio_do_real(fio, bias->bic[i].pipos);
  }
  gmx_fio_do_int(fio, bias->whebiasstop);
  /* collective variable */
  gmx_fio_do_int(fio, bias->whecv);
  gmx_fio_do_real(fio, bias->cvstrength);
  gmx_fio_do_int(fio, bias->cvnumbin);

  if (bRead){
    bias->cvbd = (double *) malloc((bias->cvnumbin+1) * sizeof(double));
  }

  gmx_fio_do_real(fio, bias->cvlow);
  gmx_fio_do_real(fio, bias->cvup);
  gmx_fio_do_real(fio, bias->cvtarget);
  gmx_fio_do_int(fio, bias->gbf);
  gmx_fio_do_int(fio, bias->lrbb);
  gmx_fio_do_int(fio, bias->signU);

  for (i = 0; i < bias->numic; i++){
    gmx_fio_do_real(fio, bias->bic[i].cvcoeff);
    gmx_fio_do_real(fio, bias->bic[i].cvcenter);
    gmx_fio_do_real(fio, bias->bic[i].cvshift);
  } 
  /* self trr */
  gmx_fio_do_int(fio, bias->selftrr);
  gmx_fio_do_int(fio, bias->selfnatom);
  gmx_fio_do_int(fio, bias->deffreq);

  /* calculate value of BBdih involved in U coordinates */
  gmx_fio_do_int(fio, bias->wheU);
  gmx_fio_do_string(fio, bias->nmu);

  /* perturbation of initial velocity */
  gmx_fio_do_int(fio, bias->perturbmethod);
  gmx_fio_do_real(fio, bias->perturbsize);
  gmx_fio_do_int(fio, bias->perturbseed);
  gmx_fio_do_int(fio, bias->perturbatom);

  /* sfree BIC *bic for grompp */
  if (!bRead){
    fprintf(stderr, "  HL grompp sfree BIC *bic\n");
    sfree(bias->bic);
  }
                        ...
                        int file_version)
{
    ...
    /* added by Huiyu Li, bias force */
    {
      do_bias_ir(fio, &(ir->bias), bRead, file_version);
    }
    /* end of adding by Huiyu Li */
}
