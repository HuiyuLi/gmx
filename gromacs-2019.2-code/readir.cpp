...
static int str_nelem(const char *str,int maxptr,char *ptr[])
{
  ...
  while (*copy != '\0') {
    if (np >= maxptr)
      fprintf(stderr, "Too many groups on line: '%s' (max is %d)",
		      str,maxptr);
    if (ptr)
      ptr[np]=copy;
    np++;
...
static void sep_string(char currline[], char *sepstring[], int maxcount, const char *spec, 
                       std::vector<t_inpfile> *inp, int expectedcount)
{
  int numstring;

  setStringEntry(inp, spec, currline, nullptr);
  fprintf(stderr, "\nread %s %s\n", spec, currline);
  numstring = str_nelem(currline, maxcount, sepstring); 

  if (numstring != expectedcount){
    fprintf(stderr, "\nHL error: mdp file %s shoud count %5d actual %5d\n\n ", 
            spec, expectedcount, numstring);
    exit(1);
  }
}

static void read_biaspara(std::vector<t_inpfile> *inp,
                          BIAS *bias, warninp_t wi)
{
  /* in both mdp file and code, we use radian as unit for dihedral */
  char *currline;
  char **sepstring;
  int i, j, count;
  int nad = 4; /* each dihedral involves 4 atoms */
  int hlmaxrep = 10;

  fprintf(stderr, "\n===========\nHL begin reading bias parameters in mdp file\n");
  printStringNewline(inp, "Huiyu Li, add bias force");

  currline = (char *) malloc(100000 * sizeof(char));
  /* number of dihedrals */
  bias->numic = get_eint(inp, "numic", 0, wi);
  fprintf(stderr, "  HL numic %5d\n", bias->numic);

  /* malloc space for BIC *bic */
  fprintf(stderr, "  HL malloc space for BIC *bic\n");
  sepstring = (char **) malloc(hlmaxrep * bias->numic * sizeof(char *));
  snew(bias->bic, bias->numic);
  fprintf(stderr, "  HL finish malloc for bic in readir.c\n");

  /* type of dihedrals */
  sep_string(currline, sepstring, bias->numic, "ictype", inp, bias->numic);
  for (i = 0; i < bias->numic; i++){
    strcpy(bias->bic[i].typeic, sepstring[i]);
    fprintf(stderr, "  HL bic typeic %5d %s\n", i, bias->bic[i].typeic);
  }

  /* atoms involved in dihedral, -1 since C index starts from 0 */
  sep_string(currline, sepstring, nad * bias->numic, "atom", inp, nad * bias->numic);
  count = 0;
  for (i = 0; i < bias->numic; i++){
    for (j = 0; j < nad; j++){
      bias->bic[i].atom[j] = atoi(sepstring[count]) - 1;
      count++;
      fprintf(stderr, "  HL bic atom %5d %5d\n", i, bias->bic[i].atom[j]);
    }
  } 

  /* range of bias bins for each dihedral */
  sep_string(currline, sepstring, 2 * bias->numic, "range", inp, 2 * bias->numic);
  count = 0;
  for (i = 0; i < bias->numic; i++){
    bias->bic[i].low = atof(sepstring[count]);
    count++;
    bias->bic[i].up = atof(sepstring[count]);
    count++;
    fprintf(stderr, "  HL range %5d %lf %lf\n", i, bias->bic[i].low, bias->bic[i].up);
  }

  /* number of bias bins */
  sep_string(currline, sepstring, bias->numic, "numbin", inp, bias->numic);
  for (i = 0; i < bias->numic; i++){
    bias->bic[i].numbin = atoi(sepstring[i]);
    fprintf(stderr, "  HL numbin %5d %5d\n", i, bias->bic[i].numbin);
  }

  /* strength of bias force */
  sep_string(currline, sepstring, bias->numic, "strength", inp, bias->numic);
  for (i = 0; i < bias->numic; i++){
    bias->bic[i].strength = atof(sepstring[i]);
    fprintf(stderr, "  HL strength %5d %5d\n", i, bias->bic[i].strength);
  }
 
  /* direction of bias force */
  sep_string(currline, sepstring, bias->numic, "direction", inp, bias->numic);
  for (i = 0; i < bias->numic; i++){
    bias->bic[i].direction = atoi(sepstring[i]);
    fprintf(stderr, "  HL direction %5d %5d\n", i, bias->bic[i].direction);
  }
 
  /* conditional bias, index of depending dihedral */
  sep_string(currline, sepstring, bias->numic, "cbic", inp, bias->numic);
  for (i = 0; i < bias->numic; i++){
    bias->bic[i].cbic = atoi(sepstring[i]);
    fprintf(stderr, "  HL cbic %5d %5d\n", i, bias->bic[i].cbic);
  }

  /* conditional bias, range of depending dihedral */
  sep_string(currline, sepstring, 2 * bias->numic, "cbicrange", inp, 2*bias->numic);
  count = 0;
  for (i = 0; i < bias->numic; i++){
    bias->bic[i].lowcbic = atof(sepstring[count]);
    count++;
    bias->bic[i].upcbic = atof(sepstring[count]);
    count++;
    fprintf(stderr, "  HL cbicrange %5d %lf %lf\n", i, bias->bic[i].lowcbic, bias->bic[i].upcbic);
  }

  /* order parameter */
  sep_string(currline, sepstring, bias->numic, "od", inp, bias->numic);
  for (i = 0; i < bias->numic; i++){
    bias->bic[i].od = atoi(sepstring[i]);
    fprintf(stderr, "  HL od %5d %5d\n", i, bias->bic[i].od);
  }

  /* range of order parameters in target basin */
  sep_string(currline, sepstring, 2 * bias->numic, "odrange", inp, 2*bias->numic);
  count = 0;
  for (i = 0; i < bias->numic; i++){
    bias->bic[i].odlow = atof(sepstring[count]);
    count++;
    bias->bic[i].odup = atof(sepstring[count]);
    count++;
    fprintf(stderr, "  HL odrange %5d %lf %lf\n", i, bias->bic[i].odlow, bias->bic[i].odup);
  }

  /* whether stop simulation when system reaches target basin, according to order parameters */
  bias->whebiasstop = get_eint(inp, "whebiasstop", 0, wi);

  /* range that close to -Pi or Pi boundaries, during which bias force will be set to zero */
  sep_string(currline, sepstring, 2 * bias->numic, "pirange", inp, 2*bias->numic);
  count = 0;
  for (i = 0; i < bias->numic; i++){
    bias->bic[i].pineg = atof(sepstring[count]);
    count++;
    bias->bic[i].pipos = atof(sepstring[count]);
    count++;
    fprintf(stderr, "  HL pirange %5d %lf %lf\n", i, bias->bic[i].pineg, bias->bic[i].pipos);
  } 

  /* collective varible */
  bias->whecv = get_eint(inp, "whecv", 0, wi);
  fprintf(stderr, "  HL whecv %5d\n", bias->whecv);

  /* strength of bias force on collective variable */
  bias->cvstrength = get_ereal(inp, "cvstrength", 0, wi);
  fprintf(stderr, "  HL cvstrength %5d\n", bias->cvstrength);

  /* number of bias bins for collective varible */ 
  bias->cvnumbin = get_eint(inp, "cvnumbin", 0, wi);
  fprintf(stderr, "  HL cvnumbin %5d\n", bias->cvnumbin);

  /* range of collective varibles */
  sep_string(currline, sepstring, 2, "cvrange", inp, 2);
  bias->cvlow = atof(sepstring[0]);
  bias->cvup = atof(sepstring[1]);
  fprintf(stderr, "  HL cvrange %lf %lf\n", bias->cvlow, bias->cvup);  

  /* coefficient of each dihedral in collective variable */ 
  sep_string(currline, sepstring, bias->numic, "cvcoeff", inp, bias->numic);
  for (i = 0; i < bias->numic; i++){
    bias->bic[i].cvcoeff = atof(sepstring[i]);
    fprintf(stderr, "  HL cvcoeff %5d %lf\n", i, bias->bic[i].cvcoeff);
  }

  /* target value for each dihedral in collective variable */
  sep_string(currline, sepstring, bias->numic, "cvcenter", inp, bias->numic);
  for (i = 0; i < bias->numic; i++){
    bias->bic[i].cvcenter = atof(sepstring[i]);
    fprintf(stderr, "  HL cvcenter %5d %lf\n", i, bias->bic[i].cvcenter);
  }

  /* shift value for dihedral in collective variable */
  sep_string(currline, sepstring, bias->numic, "cvshift", inp, bias->numic);
  for (i = 0; i < bias->numic; i++){
    bias->bic[i].cvshift = atof(sepstring[i]);
    fprintf(stderr, "  HL cvshift %5d %lf\n", i, bias->bic[i].cvshift);
  }
 
  /* target value for collective variable */
  bias->cvtarget = get_ereal(inp, "cvtarget", -100.0, wi);
  fprintf(stderr, "  HL cvtarget %lf\n", bias->cvtarget); 
 
  /* gbf, generalized bias force analytical expression option */
  bias->gbf = get_eint(inp, "gbf", -100, wi);
  fprintf(stderr, "  HL gbf %5d\n", bias->gbf);

  /* lrbb, left/right bin boundary, which is used as equilibrium location of CV in each bias bin */
  bias->lrbb = get_eint(inp, "lrbb", 0, wi);
  fprintf(stderr, "  HL lrbb %5d\n", bias->lrbb);

  /* signU, sign of U, 1 for along U direction, -1 for opposite direction */
  bias->signU = get_eint(inp, "signU", 0, wi);
  fprintf(stderr, "  HL signU %5d\n", bias->signU);

  /* parameters for output protein atoms into a new trr file */
  printStringNewline(inp, "Huiyu Li, output atom 0~(selfnatom-1) into protein.trr");

  /* selftrr, whether output self trr or not, 1 for yes, 0 for no */
  bias->selftrr = get_eint(inp, "selftrr", 0, wi);
  fprintf(stderr, "  HL selftrr %5d\n", bias->selftrr);

  /* number of atoms output into self trr */
  bias->selfnatom = get_eint(inp, "selfnatom", 0, wi);
  fprintf(stderr, "  HL selfnatom %5d\n", bias->selfnatom);

  /* deffreq, control output frequency of Gromacs trr */
  bias->deffreq = get_eint(inp, "deffreq", 1, wi);
  fprintf(stderr, "  HL deffreq %5d\n", bias->deffreq);

  /* whether calculate value of BBdih involved in U coordinates */
  bias->wheU = get_eint(inp, "wheU", 0, wi);
  fprintf(stderr, "  HL wheU %5d\n", bias->wheU);
  /* name of file, which involves atom list of BBdih involved in U coordinates*/
  setStringEntry(inp, "nameUatom", bias->nmu, nullptr);
  fprintf(stderr, "  HL nameUatom %s\n", bias->nmu);

  /* perturbation method */
  bias->perturbmethod = get_eint(inp, "perturbmethod", 0, wi);
  fprintf(stderr, "  HL perturbmethod %5d\n", bias->perturbmethod);
  /* perturbation size */
  bias->perturbsize = get_ereal(inp, "perturbsize", 0.0, wi);
  fprintf(stderr, "  HL perturbsize %lf\n", bias->perturbsize);
  /* random seed for perturbation */
  bias->perturbseed = get_eint(inp, "perturbseed", 0, wi);
  fprintf(stderr, "  HL perturbseed %d\n", bias->perturbseed);
  /* number of atoms for perturbation, -1 for all atoms */
  bias->perturbatom = get_eint(inp, "perturbatom", 0, wi);
  fprintf(stderr, "  HL perturbatom %d\n", bias->perturbatom);

  printStringNewline(inp, "Huiyu Li, end of bias force parameters");
  printStringNewline(inp, " ");

  /* free malloc space */
  fprintf(stderr, "  HL free space\n");
  free(sepstring);
  free(currline);

  fprintf(stderr, "\nHL end of reading bias parameters in mdp file\n===========\n\n");
}
            ...
            WriteMdpHeader writeMdpHeader, warninp_t wi)
{
    ...
    /* added by Huiyu Li, Jun 23 2022, add bias, explicit solvent, MPI */
    read_biaspara(&inp, &(ir->bias), wi); 
    /* end of adding by Huiyu Li */
